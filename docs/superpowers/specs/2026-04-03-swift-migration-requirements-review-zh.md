...
- **变更前**：6 个翻译服务商（AI、DeepL、Google、Microsoft、Bing Web、Google Web）
- **变更后**：仅使用 AI 翻译（通过已配置的 LLM）
- **影响**：移除 5 个服务文件，简化 PTAIServices/Translation/
- **架构**：`AITranslator` 结构体取代 `TranslationProvider` 协议层级

### 变更 2：Embedding — 移除本地/Ollama

- **变更前**：远程 API + 本地 Ollama + FTS5 降级方案
- **变更后**：仅保留远程 API + FTS5 降级方案（Ollama 在 iOS 上不可行）
- **影响**：简化 EmbeddingService；FTS5 提供离线搜索能力
- **架构**：`EmbeddingService` 仅使用远程服务商；FTS5 作为离线降级方案

### 变更 3：UI — 莫兰迪色系

- **变更前**：系统颜色 + 自定义 AccentColor
- **变更后**：莫兰迪色系（低饱和度莫兰迪色系）作为主要设计语言
- **影响**：所有 UI 元素使用柔和大地色调；Settings 页面重新设计为原生 Form/Section/List
- **架构**：`PTColors` 枚举定义莫兰迪色板；PTUI 组件统一采用

### 变更 4：AI Settings — 重构布局

- **变更前**：AI 设置分散在多个页面
- **变更后**：6 个逻辑分区：服务商管理、对话行为、工具与 Agent、知识与记忆、多模态、翻译
- **影响**：提升可发现性；所有可配置选项均已暴露
- **架构**：PTFeatures/Settings/AISettings/ 重构为基于分区的布局

### 变更 5：PDF 增强 — 完整 AI 集成

- **变更前**：PDF 渲染 + 基本文本提取
- **变更后**：完整的 PDF 内容桥接（PDFKit 文本提取、Vision OCR 处理扫描版 PDF），AI 工具/RAG/翻译对 EPUB 和 PDF 的处理完全一致
- **影响**：PTReader 中新增 `PDFContentBridge`；`BookContentBridge` 协议统一 EPUB/PDF
- **架构**：已添加至 PTReader/PDF/ 和 PTReader/Common/

### 变更 6：书籍导入 — 目录扫描模式

- **变更前**：仅支持沙盒导入（复制到应用容器）
- **变更后**：双模式 — 沙盒导入 + 目录扫描（Security-Scoped Bookmarks、原地阅读、自动发现）
- **影响**：新增 `DirectoryScanner` 服务；书签持久化；文件监控
- **架构**：已添加至 PTFeatures/Bookshelf/，书签辅助工具位于 PTCore/Utils/
...
### 可行性得分：高
所有需求在拟议架构下均可行。9 个原始缺口 + 6 项评审后变更均已提供具体解决方案。
...
需求文档与架构设计完整且一致。所有功能均已列举，验收标准已定义，并已与源代码进行交叉验证。9 个已识别的缺口和 6 项评审后变更均有具体解决方案。
