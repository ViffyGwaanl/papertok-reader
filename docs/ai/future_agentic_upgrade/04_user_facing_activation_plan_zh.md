...
- AI Chat 左下角 `+` 打开 Add-to-Chat sheet；其中 `AI 研讨会` 会在当前 AI Chat 页面内展开 Seminar runtime 面板，不等同于 `Choose style / 选择风格`；`Choose style / 选择风格` 内的 `研讨会设置` 只打开 Seminar 配置页，不会切换当前 active skill。当前内嵌面板复用现有 runtime；`AiSeminarDirectorState` 第一片已可随 runtime state 保存，并能把 completed run 中的 open question 标为 `askUser`、disagreement 标为 `refreshEvidence` 且在状态区显示主持人下一步；`askUser` 状态下用户可以输入回复并选择让角色回应、重新找证据或整理总结，回复只保存为 human intervention，不进入 formal evidence；但还不是可持久化的 Chat message part，也没有真正自动执行多轮证据刷新或模型反驳轮。
...
| UFA-C02-T19 | Ready | Multi-round evidence refresh | 讨论出现分歧或证据不足时，Director 可以重新检索并进入反驳轮。 | UFA-C02-T17, UFA-C07-T05 | Director loop service、contradiction scan、evidence refresh tests | 至少覆盖初始证据、第一轮角色观点、分歧扫描、按 gap 重新检索、反驳和 synthesis；每次 evidence refresh 都有 SourceRef；默认 current book first，默认不开 web。 |
| UFA-C02-T20 | In Review slice | User-in-the-loop Seminar | 用户能在 AI Chat 中插话、指定追问角色或回答澄清问题。 | UFA-C02-T17, E07 Chat UI | `AiSeminarUserIntervention`、`AiSeminarRuntimeNotifier.recordUserIntervention`、`AiSeminarRuntimePanel` reader turn UI、tests | 第一片已接入：Director 可进入 `needsUserInput`；页面显示用户回复输入框、目标角色选择、让角色回应、重新找证据和整理总结动作；用户输入保留为 `lastUserIntervention` human turn，不写 `evidenceBundle`、不当作 AI evidence；提交后只更新下一步 intent。仍需把该 intent 接到真实 role execution、自动 evidence refresh 和 Chat run card。 |
| UFA-C02-T21 | In Review slice | AI Chat Seminar run card | 在 AI Chat 内展示多角色讨论，而不是强制跳到独立 Seminar 页面。 | UFA-C02-T17, UFA-C02-T20 | `AiSeminarRuntimePanel`、AI Chat inline panel entry、Seminar runtime page compatibility、l10n | 已完成第一片：AI Chat `+` -> `AI 研讨会` 在当前 AI Chat 页面内展开 Seminar runtime panel，带入输入框问题，不改变 active skill；阅读页选中文本 `研讨` 也已进入阅读页 AI Chat 的内嵌 runtime panel，带入 reader SourceRef；面板可关闭，也可跳到完整 runtime page；旧 `Choose style -> 研讨会设置` 路径仍只打开配置页。仍需持久化 AI Chat message part、多轮分歧/证据刷新、用户插话、分歧 tab 和 per-run 多实例隔离。 |
