...
| Sync / Export / Remote Preview 知识资产 | `Settings -> AI -> Knowledge sync/export / 知识同步 / 导出`。 | 本分支已接入安全 manifest 预览、Markdown 学习导出、HTML study report、Anki TSV 导出、机器可读 sync bundle、创建入口、`Send conflicts to Review` 冲突 handoff、`Preview remote sync` 远端 bundle 预览、`Send remote incoming to Review` 安全远端 KnowledgeCard 导入、`Send remote conflicts to Review` 远端冲突 handoff、受保护 `Upload sync bundle` 写出，以及 Review inbox 直达入口；只纳入已应用 KnowledgeCard 和复习历史，显性显示排除项、待审冲突和远端 incoming/outgoing/conflict 计数；provider 级闭环覆盖安全的本地 KnowledgeCard 冲突进入 Review Inbox 后 approve/apply，并解除 pending conflict 回到 export included 集合。 | 目前是本地导出 + 远端 bundle 预览 + 安全远端 KnowledgeCard Review 导入 + 安全 bundle 写出 + 冲突 Review handoff；只有本地 staged 且满足 `knowledge-card + schemaVersion=1 + 无 secret payload + 有可追踪 SourceRef` 的冲突可 apply，远端 incoming KnowledgeCard 会降级为 pending candidate，远端 preview 冲突只支持 dismiss/triage；写出前如果发现远端 incoming/conflict 会阻止覆盖；不是完整云同步引擎，不执行双向自动合并或失败回滚。 |
...
21. 用户点击 `Send remote conflicts to Review / 发送远端冲突到 Review`。
22. 系统把远端冲突创建为 `ReviewItem(sourceType=sync-conflict, status=pending)`；ReviewItem 只保存安全 metadata、payload key 列表、SourceRef count 和 SourceRef safe JSON，不保存 remote raw payload value。
23. 远端 preview 产生的 `sync-conflict` 只用于 triage/dismiss，不显示 approve/apply；只有远端冲突 staging/import/writeback 执行器把远端 envelope 安全落到本地待解析冲突区后，才允许进入可恢复 apply 流。
24. 用户点击 `Upload sync bundle / 上传 sync bundle`。
25. 系统重新生成本机安全 sync bundle；如果远端 bundle 不存在，创建远端目录并上传；如果远端 bundle 已存在，先执行 preview gate，只有 `incoming=0` 且 `conflict=0` 时才允许覆盖写出。
26. 如果远端存在 incoming 或 conflict，上传中止并显示错误，不删除远端内容、不覆盖本地资产、不把远端冲突自动应用到本地。
...
- 当前入口创建本地 manifest、Markdown 学习导出、HTML study report、Anki TSV、sync bundle，能把安全远端 incoming KnowledgeCard 和本地/远端冲突送入 Review Inbox，并能在无远端 incoming/conflict 时上传安全 sync bundle；仅支持安全 KnowledgeCard 本地冲突的用户确认恢复，不执行双向自动合并、不自动同步备份。
...
| --- | --- | --- | --- |
| 完整云同步引擎 | 当前已有本地导出、机器可读 sync bundle、远端 bundle preview、安全远端 incoming KnowledgeCard Review 导入、受保护 bundle 上传、安全冲突 Review handoff 和安全 KnowledgeCard 冲突本地恢复；还没有远端冲突 staging/import、双向自动合并、review history 导入和失败回滚执行器。 | 设计并实现远端冲突合并器、review history staging/import、rollback 和同步状态 UI。 | API key 永不同步；冲突进入 Review；不得使用 whole-file newer-wins 覆盖用户资产。 |
