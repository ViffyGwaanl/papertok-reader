...
- AI Chat 左下角 `+` 打开 Add-to-Chat sheet；其中 `AI 研讨会` 会在当前 AI Chat 页面内展开 Seminar runtime 面板，不等同于 `Choose style / 选择风格`；`Choose style / 选择风格` 内的 `研讨会设置` 只打开 Seminar 配置页，不会切换当前 active skill。当前内嵌面板复用现有 runtime；`AiSeminarDirectorState` 第一片已可随 runtime state 保存，并能把 completed run 中的 open question 标为 `askUser`、disagreement 标为 `refreshEvidence` 且在状态区显示主持人下一步，但还不是可持久化的 Chat message part，也没有真正自动执行多轮证据刷新或用户插话调度。
...
| UFA-C02-T20 | Ready | User-in-the-loop Seminar | 用户能在 AI Chat 中插话、指定追问角色或回答澄清问题。 | UFA-C02-T17, E07 Chat UI | Seminar user turn model、composer routing、tests | Director 可进入 `needsUserInput`；用户输入保留为 human turn，不当作 AI evidence；用户可选择“让某角色回应”“重新找证据”“继续总结”；取消和重试仍按 active job id 处理。 |
