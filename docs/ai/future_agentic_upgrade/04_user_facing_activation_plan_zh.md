...
| 选中文本 -> AI Seminar | 阅读页选中文本 -> `研讨`，或 `Settings -> AI -> Seminar Mode / 研讨会模式`。 | 本分支已接入结构化 runtime：用户可启动 role-by-role Seminar，查看 evidence、角色输出、Shared Whiteboard、synthesis，并把 traceable synthesis、候选卡和候选 flashcard 送入 Review Inbox；Seminar 页面会在启动前显示 `Provider readiness`，列出当前 provider、model、context/max output、Tools/Vision/Thinking 能力、Streaming 状态未知提示和成本未知原因；角色完成后显示 `Local token estimate`、input/output 估算和 `Provider billing may differ` 提示；页面提供本地 `Role output token budget` 和 `Run token budget` guardrails；页面可在同一书籍/同一入口问题恢复本机保存的 completed/cancelled/failed Seminar state，并显示 `Recovered local Seminar state`。 | 阅读页优先 current book evidence；Settings 独立入口没有 current book 时会走 library fallback。Seminar synthesis 本身只进入 Review，不自动应用；候选卡和候选 flashcard 仍需用户在 Review Inbox 中批准/应用后才成为长期资产或复习项；provider readiness 只读本地 Provider Center 配置和 capability cache，不记录 API key；本地 token 估算和本地预算不等于真实 provider usage，也不估算缺少 pricing/usage metadata 的美元成本；重启前仍在 running 的 Seminar 只恢复为 interrupted/retryable，不伪装继续后台生成；换书或换选区打开 Seminar 会丢弃旧 runtime/cache，不显示不属于当前入口的旧研讨。 |
...
- 角色完成后必须显示本地 token 估算和 `Provider billing may differ` 提示；该估算只能来自本地 prompt/evidence/response 字符计数，不得伪装成 provider billing。
...
- 页面必须说明美元 cost cap 在 provider usage/pricing metadata 接入前不可用；不得把本地 token budget 当作真实账单上限。
...
| Seminar 真实 provider usage、美元成本上限、后台续跑 | Seminar runtime 已能流式、取消、重试、Review handoff，并显示 provider readiness、capability cache、成本未知原因、本地 token 估算、本地 role/run token budget 和本机 state 恢复；running state 重启后恢复为 interrupted/retryable。未接真实 provider usage 汇总、美元成本上限和真正后台续跑。 | 接入 provider usage metadata、pricing metadata、美元成本上限、后台任务队列和恢复状态。 | 移动资源 gate；长任务可取消、失败可恢复或重试；无 pricing/usage metadata 时继续显示成本未知原因；本地 token 估算不得当作账单；本地 token budget 不得声明为 provider billing cap；不能把本机 recovery cache 当作同步资产。 |
