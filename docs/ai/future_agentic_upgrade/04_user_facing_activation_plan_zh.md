...
| UFA-C02-T21 | In Review slice | AI Chat Seminar run card | 在 AI Chat 内展示多角色讨论，而不是强制跳到独立 Seminar 页面。 | UFA-C02-T17, UFA-C02-T20 | `AiSeminarRuntimePanel`、AI Chat inline panel entry、Seminar runtime page compatibility、l10n | 已完成第一片：AI Chat `+` -> `AI 研讨会` 在当前 AI Chat 页面内展开 Seminar runtime panel，带入输入框问题，不改变 active skill；面板可关闭，也可跳到完整 runtime page；旧 `Choose style -> 研讨会设置` 路径仍只打开配置页。仍需持久化 AI Chat message part、多轮分歧/证据刷新、用户插话、分歧 tab 和 per-run 多实例隔离。 |
| UFA-C02-T22 | Ready | Seminar entry migration | 阅读页和 AI Chat 的 Seminar 入口默认进入 Chat 内嵌 run，独立页只作为详情/恢复入口。 | UFA-C02-T21 | entry router、compatibility tests | `研讨`、AI Chat `AI 研讨会`、`研讨会设置` 的行为边界清晰；旧独立页面读取同一 runtime state；历史恢复、queued job、budget、Review handoff 不分叉。 |
