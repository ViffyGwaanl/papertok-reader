...
- AI Chat 左下角 `+` 打开 Add-to-Chat sheet；其中 `AI 研讨会` 会在当前 AI Chat 页面内展开 Seminar runtime 面板，不等同于 `Choose style / 选择风格`；`Choose style / 选择风格` 内的 `研讨会设置` 只打开 Seminar 配置页，不会切换当前 active skill。当前内嵌面板复用现有 runtime；`AiSeminarDirectorState` 第一片已可随 runtime state 保存，并能把 completed run 中的 open question 标为 `askUser`、disagreement 标为 `refreshEvidence` 且在状态区显示主持人下一步；`askUser` 状态下用户可以输入回复并选择让角色回应、重新找证据或整理总结，回复只保存为 human intervention，不进入 formal evidence；其中“让角色回应”已会调用所选角色生成 follow-up turn 并更新 synthesis，“重新找证据”已会重新检索 evidence、重跑角色并更新 synthesis，“整理总结”已会用现有 evidence 和 turns 执行本地 synthesis 并收束 Director；但还不是可持久化的 Chat message part，也没有真正自动按分歧缺口刷新证据或完整分歧反驳 loop。
...
| UFA-C02-T19 | In Review slice | Multi-round evidence refresh | 讨论出现分歧或证据不足时，Director 可以重新检索并进入反驳轮。 | UFA-C02-T17, UFA-C07-T05 | Director loop service、contradiction scan、evidence refresh tests | 已完成用户触发的 evidence refresh 执行路径：用户在 `askUser` 状态点击“重新找证据”后，runtime 会重新调用 evidence broker、保留 current book first、重跑角色并更新 synthesis，每次刷新后的 evidence 仍需 SourceRef。仍需覆盖自动分歧扫描、按 gap 重新检索、反驳轮和最终 synthesis；默认不开 web。 |
