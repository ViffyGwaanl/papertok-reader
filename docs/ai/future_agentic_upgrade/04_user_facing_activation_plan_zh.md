...
- 默认 backend 仍是 exact backend；替换成 sqlite-vec/ANN backend 时必须保持 SourceRef、chunk evidence、RRF/hybrid/MMR/rerank contract。
