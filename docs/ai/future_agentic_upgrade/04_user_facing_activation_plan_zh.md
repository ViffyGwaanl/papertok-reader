...
| Sync / Export / Remote Preview 知识资产 | `Settings -> AI -> Knowledge sync/export / 知识同步 / 导出`。 | 本分支已接入安全 manifest 预览、Markdown 学习导出、HTML study report、Anki TSV 导出、机器可读 sync bundle、创建入口、`Send conflicts to Review` 冲突 handoff、`Preview remote sync` 远端 bundle 预览、`Send remote conflicts to Review` 远端冲突 handoff，以及 Review inbox 直达入口；只纳入已应用 KnowledgeCard 和复习历史，显性显示排除项、待审冲突和远端 incoming/outgoing/conflict 计数；provider 级闭环覆盖安全的本地 KnowledgeCard 冲突进入 Review Inbox 后 approve/apply，并解除 pending conflict 回到 export included 集合。 | 目前是本地导出 + 远端 bundle 预览 + 冲突 Review handoff；只有本地 staged 且满足 `knowledge-card + schemaVersion=1 + 无 secret payload + 有可追踪 SourceRef` 的冲突可 apply，远端 preview 冲突只支持 dismiss/triage；不是完整云同步引擎，不执行远端写回、双向自动合并或失败回滚。 |
...
- 当前入口创建本地 manifest、Markdown 学习导出、HTML study report、Anki TSV、sync bundle，并能把本地/远端冲突送入 Review Inbox；仅支持安全 KnowledgeCard 本地冲突的用户确认恢复，不执行远端写回、不执行跨设备自动合并、不自动同步备份。
...
| --- | --- | --- | --- |
| 完整云同步引擎 | 当前已有本地导出、机器可读 sync bundle、远端 bundle preview、安全冲突 Review handoff 和安全 KnowledgeCard 冲突本地恢复；还没有远端写回、双向自动合并和失败回滚执行器。 | 设计并实现 per-entity remote writeback、远端冲突合并器、staging/rollback 和同步状态 UI。 | API key 永不同步；冲突进入 Review；不得使用 whole-file newer-wins 覆盖用户资产。 |
