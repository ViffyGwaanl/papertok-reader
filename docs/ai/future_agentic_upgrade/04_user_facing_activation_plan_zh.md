...
- AI Chat 左下角 `+` 打开 Add-to-Chat sheet；其中 `AI 研讨会` 会在当前 AI Chat 页面内展开 Seminar runtime 面板，不等同于 `Choose style / 选择风格`；`Choose style / 选择风格` 内的 `研讨会设置` 只打开 Seminar 配置页，不会切换当前 active skill。当前内嵌面板复用现有 runtime，不是可持久化的 Chat message part，也不是多轮 Seminar Director Loop。
...
| UFA-C02-T17 | Ready | Chat Seminar DirectorState | 把 Seminar 从固定角色顺序推进为 AI Chat 可恢复 Director loop。 | UFA-C02-T16, E01-C05-T01 | `AiSeminarDirectorState`、state migration、tests | 记录轮次、已发言摘要、证据刷新次数、分歧列表、用户插话状态和下一步 intent；恢复时跳过已完成角色；不得生成第二套 Seminar runtime state。 |
