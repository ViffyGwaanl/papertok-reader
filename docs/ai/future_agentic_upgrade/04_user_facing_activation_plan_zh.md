...
- AI Chat 左下角 `+` 打开 Add-to-Chat sheet；其中 `AI 研讨会` 当前仍打开独立 Seminar runtime，不等同于 `Choose style / 选择风格`；`Choose style / 选择风格` 内的 `研讨会设置` 只打开 Seminar 配置页，不会切换当前 active skill。AI Chat 内嵌多轮 Seminar Director Loop 是下一组任务，当前不能描述为已可用。
...
| AI Chat 内嵌多轮 Seminar | 当前 AI Chat 已有 `AI 研讨会` 入口和 `研讨会设置` 入口，但打开的是独立 Seminar runtime；Choose style 仍主要负责选择 active skill，不承载多角色讨论的渲染、证据刷新、用户插话和分歧回合。现有 Seminar 角色 prompt 不是完整的用户可编辑 role profile；讨论仍以固定角色顺序为主，不是 OpenMAIC 式 Director 每轮调度。 | 拆 Chat 内嵌 Seminar run 卡片、DirectorState、role profile 设置、多轮 contradiction/evidence refresh、用户插话和独立页面兼容迁移。 | 同一讨论只能有一个 runtime state；AI Chat run 卡片、独立详情页和恢复缓存必须共享 SourceRef、turn ledger、budget、Review handoff；用户插话不得被当作 AI evidence；重新检索必须保留 current book first 和默认不开 web。 |
...
| UFA-C02-T18 | Ready | Seminar role prompt settings | 用户可编辑多角色提示词和角色策略。 | UFA-C02-T17, E06 governance | role profile store、Settings 和 AI Chat 配置入口 | 默认角色保留 `critical/supportive/synthesizer/verifier`；用户可改显示名、prompt、启用状态、证据策略和 token guardrail；工具越权、空 prompt、未知角色和写工具注入必须被 validator 拒绝。 |
...
| UFA-C02-T21 | Ready | AI Chat Seminar run card | 在 AI Chat 内展示多角色讨论，而不是强制跳到独立 Seminar 页面。 | UFA-C02-T17, UFA-C02-T20 | AI Chat message part、Seminar run card widgets、l10n | 同一条 run 卡片包含证据、角色发言、分歧、白板、总结和送审入口；streaming 中有进度和取消；完成后可 Send to Review；移动端不遮挡阅读正文。 |
