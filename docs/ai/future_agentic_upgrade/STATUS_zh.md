...
| K2 | 网关级可靠性(融合 new-api/CLIProxyAPI:Key 轮换冷却提炼进 kit、供应商级自动回退、自托管网关预设) | in progress | 批次 1/3a/3b 已实现:聊天+嵌入两份手写循环统一进 kit(index.dart 625→534),顺带修复"解除冷却"按钮自上线即空操作的 bug;批次 2 撤销(翻译已继承轮换,归 E2);剩批次 4(供应商级回退 opt-in UI,需用户拍板默认关+展示位置) | `briefs/K2_gateway_reliability_zh.md` |
