...
| ANN/sqlite-vec 当前书向量后端和大书性能验收 | 当前书搜索已经避免全量正文/JSON 常驻内存，并做串行、progress、取消、stale query 丢弃、background isolate scoring 和 bounded FTS/BM25 候选预筛；但它不是 sqlite-vec/Vec1/ANN 后端，候选预筛无候选时仍会回退完整分页扫描。 | `UFA-C07-T02` 继续保留 In Review，下一块补大书性能验收；若性能 gate 失败，再拆 sqlite-vec/Vec1 或 ANN backend。 |
