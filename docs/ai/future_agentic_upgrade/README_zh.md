...
- `Custom skills` 已做中文适配；中文界面中对应 `自定义技能`，英文界面仍显示 `Custom skills`；AI Chat 的 `+ -> Choose style / 选择风格` 中，自定义 skill 行也会显示 `Custom skills / 自定义技能` 配置入口，点击进入同一个导入/启用/删除页面，不会偷偷切换当前 active skill；普通内置 skill 仍是选择入口，等后续有 skill profile/override contract 后再加参数页。
