...
| AI Chat 内嵌多轮 Seminar | AI Chat 已有 `AI 研讨会` 功能卡、当前页内嵌 `AiSeminarRuntimePanel`、完整 runtime page 跳转和 `研讨会设置` 入口；Seminar settings 已可编辑角色显示名和 custom prompt；`AiSeminarDirectorState` 第一片已进入 runtime state 和本机恢复 JSON，可记录轮次、已完成角色、证据账本、白板账本、分歧 id、用户插话和下一步 intent；completed run 留下 open question 时会把下一步标为 `askUser`，留下 disagreement 且仍有轮次预算时会标为 `refreshEvidence`，status banner 会显示主持人下一步；`askUser` 状态下页面提供用户回复输入框、目标角色选择、让角色回应、重新找证据和整理总结动作，提交后保存为 human intervention，不进入 formal evidence。还没有持久化为 AI Chat message part 的可展开 run 卡片，也没有真正执行自动证据刷新、用户回复后的模型反驳轮、角色启用状态、证据策略或工具范围配置。 | 继续执行 `UFA-C02-T18` 到 `UFA-C02-T22` 的剩余部分：role profile 剩余策略、真正 evidence refresh loop、user-in-the-loop role execution、持久化 Chat run card 和入口迁移。 |
