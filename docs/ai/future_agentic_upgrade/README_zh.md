...
| 图谱可视化探索 | `Settings -> AI -> Concept graph / 概念图谱`，或阅读页选中文本 -> `图谱/Graph`。 | 本分支已接入最小 Explorer、阅读页选中文本入口和 KnowledgeCard -> draft ConceptGraph producer；可查看已有图谱、按选中文本筛选相关概念、查看局部路径、证据和 orphan/broken link。当前 producer 可从 `applied + traceable + conceptRefs` 的 KnowledgeCard 生成待审图谱关系；Seminar candidate card 可携带 `conceptRefs`，经 Review apply 后进入同一图谱候选链路；RAG/GraphRAG derived search result 已有服务层 producer，可基于 traceable chunk SourceRef 生成 draft concept relation 和 pending ReviewItem。阅读页/AI 面板尚未自动触发该 RAG/GraphRAG producer，更强可视化布局仍在剩余任务中。 |
