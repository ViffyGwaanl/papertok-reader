...
当前没有单独列出的入口级 `In Review` 证据；仍不能当作当前分支已产品化或发布版可用的内容，统一列在“当前还不能用”。
...
| 一键开启研讨 | 阅读页选中文本 -> `研讨`，或 `Settings -> AI -> Seminar Mode / 研讨会模式`。 | 本分支已接入结构化 AI Seminar runtime/UI：展示 evidence、role turns、Shared Whiteboard、synthesis，支持取消、失败重试，并可把 traceable synthesis、候选卡和候选 flashcard 送入 Review Inbox；页面级 widget 覆盖点击 `Send to Review` 后写入 pending synthesis/card/flashcard handoff；`Provider readiness` 区块会显示当前 provider、model、context/max output、Tools/Vision/Thinking 能力、Streaming 状态未知提示和成本未知原因；角色输出会显示 `Local token estimate`、input/output 估算和 `Provider billing may differ` 提示；用户可在页面设置本地 `Role output token budget` 和 `Run token budget`，超出本地估算后停止后续 Seminar 步骤；完成或中断的 Seminar state 会保存为本机恢复缓存，同一书籍/同一入口问题重新打开页面时显示 `Recovered local Seminar state`，换书或换选区会清掉旧 runtime/cache，running state 只恢复为 interrupted/retryable，不伪装后台继续执行；不自动写长期资产。 |
...
| Seminar 真实 provider usage、美元成本上限、后台续跑 | Seminar runtime 已能流式、取消、重试、送 Review，并能显示 provider/model capability、成本未知原因、本地 token 估算、本地 role/run token budget 和本机 state 恢复；running state 重启后会降级为 interrupted/retryable。未接真实 provider usage 汇总、美元成本上限和真正后台续跑。 | 接入 provider usage metadata、pricing metadata、美元成本上限、后台任务队列和移动资源 gate。 |
