...
- AI Chat 里的 `AI 研讨会` 当前打开独立 Seminar runtime；“在原 AI Chat 消息流中直接展开多角色、多轮、可插话研讨”列在当前还不能用，不应描述为已接入。
...
| AI Chat 内嵌多轮 Seminar | AI Chat 已有 `AI 研讨会` 功能卡和 `研讨会设置` 入口，但当前仍打开独立 Seminar runtime；Seminar settings 已可编辑角色显示名和 custom prompt。还没有在原 AI Chat 消息流内展示可展开 Seminar run 卡片，也没有 DirectorState、多轮分歧检测、证据刷新、用户插话/澄清回合、角色启用状态、证据策略或工具范围配置。 | 执行 `UFA-C02-T17` 到 `UFA-C02-T22`：DirectorState、role profile 剩余策略、多轮 evidence refresh、user-in-the-loop、Chat run card 和入口迁移。 |
