...
| 旧 Memory 独立页面 SourceRef 审计 | 新 KnowledgeCard/ReviewItem/SpacedReview 路径已有 SourceRef 审计；旧划线/笔记列表和搜索结果已接 `SourceRefEvidenceList` 与 jump audit；Memory 候选经 Review Inbox 已有 SourceRef，但 daily/long-term Memory 独立浏览页还没有同等来源审计入口。 | 把 Memory home、daily memory、long-term memory 浏览 UI 接到 source-specific `SourceRefEvidenceList`、jump audit 和不可跳原因展示。 |
