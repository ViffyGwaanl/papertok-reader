...
| AI Chat 内嵌多轮 Seminar | AI Chat 已有 `AI 研讨会` 功能卡和 `研讨会设置` 入口，但当前仍打开独立 Seminar runtime；还没有在原 AI Chat 消息流内展示可展开 Seminar run 卡片，也没有用户可编辑的角色 prompt/profile、多轮分歧检测、证据刷新、用户插话/澄清回合。 | 执行 `UFA-C02-T17` 到 `UFA-C02-T22`：DirectorState、role profile 设置、多轮 evidence refresh、user-in-the-loop、Chat run card 和入口迁移。 |
