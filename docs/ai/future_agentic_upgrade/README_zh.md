...
- AI Chat 里的 `AI 研讨会` 当前会在 AI 对话页内展开 Seminar runtime 面板；“持久化为 Chat 消息流中的多角色、多轮、可插话 Director 研讨卡片”列在当前还不能用，不应描述为已接入。
...
| AI Chat 内嵌多轮 Seminar | AI Chat 已有 `AI 研讨会` 功能卡、当前页内嵌 `AiSeminarRuntimePanel`、完整 runtime page 跳转和 `研讨会设置` 入口；Seminar settings 已可编辑角色显示名和 custom prompt。还没有持久化为 AI Chat message part 的可展开 run 卡片，也没有 DirectorState、多轮分歧检测、证据刷新、用户插话/澄清回合、角色启用状态、证据策略或工具范围配置。 | 继续执行 `UFA-C02-T17` 到 `UFA-C02-T22` 的剩余部分：DirectorState、role profile 剩余策略、多轮 evidence refresh、user-in-the-loop、持久化 Chat run card 和入口迁移。 |
