...
| E08-C02-T10 | 接入远端同步状态面板 | E08-C02-T05, E08-C02-T06, E07 observability | `KnowledgeRemoteSyncStatus`、`KnowledgeAssetExportState.remoteSyncStatus`、`KnowledgeAssetExportPage` remote status panel、l10n keys | 页面显示 `Not previewed / Review required / Ready to upload / Uploaded / Failed` 和下一步动作；incoming/conflict 导致 Review required；无 blocker 显示 Ready to upload；上传成功显示 Uploaded；失败显示 Failed；刷新或重新创建 manifest 清理旧上传状态；面板不执行自动合并、远端写回或 Review bypass。 |
