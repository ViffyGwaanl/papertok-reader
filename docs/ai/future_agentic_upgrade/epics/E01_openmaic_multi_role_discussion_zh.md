...
| E01-C05-T05 | 在 AI Chat 渲染 Seminar run 卡片 | E01-C05-T01, E07 Chat UI | `AiSeminarRuntimePanel` first slice; AI Chat message part / run card widgets | 已完成第四片：AI Chat `+` -> `AI 研讨会` 会在当前 AI Chat 页面内展开 runtime panel，不改 active skill，并向 `conversationV2` 写入 `seminarRunCard` meta 和兼容 fallback assistant message；历史重载后这张 `AI 研讨会` 卡片可点击重新打开同一类 inline runtime。内嵌 runtime 运行、完成或刷新证据时，会按同一个 `seminarSessionId` 回写卡片 `status/sourceRefCount/snapshot`，历史卡可直接显示证据快照、角色观点、研讨总结、分歧数和开放问题数；当卡片 session 与当前活跃 runtime 匹配且 `canSendToReview` 为 true 时，卡片内显示 `发送到待审`，只调用 Seminar Review handoff。后续仍需升级完整白板、独立送审详情子视图和 per-run 多实例隔离，且不跳转独立 Seminar 页面也能走完讨论。 |
...
| E01-C05-T11 | 升级 AI Chat Seminar run card 为完整 message part | E01-C05-T05, E01-C05-T08 | `seminarRun` message part schema + widgets + history migration | 已完成 evidence/role/synthesis snapshot 和首片 Review handoff：历史重载后仍显示证据快照、角色观点、研讨总结、分歧数和开放问题数；同 session active runtime 已完成且可送审时，卡片显示 `发送到待审` 并写 pending Review；Seminar 卡片不暴露普通 assistant 回答的 KnowledgeCard/Memory/regenerate 操作，避免伪造来源。剩余部分是完整白板子视图、历史卡离线送审详情子视图、run-scoped composer 子视图和 per-run 多实例隔离。 |
