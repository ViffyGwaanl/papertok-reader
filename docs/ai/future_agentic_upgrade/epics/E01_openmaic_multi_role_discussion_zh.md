...
| E01-C05-T11 | 升级 AI Chat Seminar run card 为完整 message part | E01-C05-T05, E01-C05-T08 | `seminarRun` message part schema + widgets + history migration | 已完成 evidence/role/synthesis snapshot、白板正文首片、首片 Review handoff、scoped runtime 读取和 run-scoped 读者回合 composer：历史重载后仍显示证据快照、角色观点、研讨总结、分歧数、开放问题数、`研讨白板` 中的分歧和开放问题正文；同 session scoped runtime 已完成且可送审时，卡片显示 `发送到待审` 并写 pending Review；completed 卡片显示 `读者参与`，可点名角色回应、重新找证据或整理总结；Seminar 卡片不暴露普通 assistant 回答的 KnowledgeCard/Memory/regenerate 操作，避免伪造来源。剩余部分是历史卡离线送审详情子视图、分歧 tab 和完整 message part schema migration。 |
