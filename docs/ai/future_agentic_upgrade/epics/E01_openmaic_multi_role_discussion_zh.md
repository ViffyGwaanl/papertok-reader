...
| E01-C05-T03 | 接入多轮分歧与证据刷新 | E01-C02-T02, E01-C05-T01 | Director loop service | 已完成第一片：用户触发和 disagreement 预算内自动触发都会重新检索 evidence、重跑角色并保留 SourceRef；刷新后仍有分歧且预算耗尽时转为 `askUser`。后续继续补结构化 contradiction gap scan、角色 rebuttal turn 和持久化 Chat run card。 |
