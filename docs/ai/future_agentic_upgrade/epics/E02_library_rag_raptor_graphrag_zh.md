...
- 当前书语义检索已接入同一类 `AiVectorSearchBackend`，优先按 `bookId` 做语义召回；Vec1 builder 已能为 provider/model/dim 建全局 ANN 表，并为每本书建 per-book sidecar ANN 表。current-book 查询只使用完整的 per-book ANN，不使用全局 ANN topK 后过滤，也不使用无预算 `vector_full_scan` native path；FTS/BM25 继续承担精确文本候选，应用层合并时会按 chunk 去重，分页扫描只作为保底降级。
