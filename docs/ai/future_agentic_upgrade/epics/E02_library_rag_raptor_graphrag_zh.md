...
- 当前书语义检索已接入同一类 `AiVectorSearchBackend`，优先按 `bookId` 做语义召回；Vec1 builder 已能为 provider/model/dim 建全局 ANN 表，并为每本书建 per-book sidecar ANN 表，status API 会单独报告 sidecar ready/missing。current-book 查询只使用完整的 per-book ANN，不使用全局 ANN topK 后过滤，也不使用无预算 `vector_full_scan` native path；FTS/BM25 继续承担精确文本候选，应用层合并时会按 chunk 去重，分页扫描只作为保底降级。
...
| E02-C02-T09 | 接入 current-book vector backend recall | E02-C02-T01 | `SemanticSearchCurrentBook.vectorSearch`, `AiVectorSearchBackend.bookId`, per-book Vec1 sidecar | 当前书检索先按 `bookId` 调用 book-scoped 向量后端，并把 fallback scan budget 传给后端；per-book Vec1 ANN 表存在且 shadow row 完整时先用 ANN，缺表或不完整时降级；status/API/UI 必须把 per-book sidecar 缺口和全局 ANN group 缺口分开显示；全局 Vec1 ANN 和无预算 `vector_full_scan` native path 在 `bookId` 场景跳过；ANN/FTS 命中同一 chunk 时应用层只保留一条证据。 |
...
- 不在缺少平台 extension/package、build job UI 和 provider/model 失效 gate 时，把 ANN/native vector 宣称为发布级生产能力；默认检索路径必须保留 exact fallback。
- 不默认同步 `ai_index.db`。
