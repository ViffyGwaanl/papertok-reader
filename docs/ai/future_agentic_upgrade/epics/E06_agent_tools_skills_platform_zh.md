...
- `AiSeminarRuntimeNotifier` 已把 Seminar completed/cancelled/failed state 保存为 `aiSeminarRuntimeStateV1` 本机恢复缓存；恢复只在同一书籍/同一入口问题显示，换书或换选区会清理旧 runtime/cache；restored running state 只有在存在连续、证据可追踪且 provider/model/pricing 匹配当前配置的 completed role prefix 时才从下一个缺失角色继续，并复用已保存 evidence；只有 active partial stream、无 completed turn、checkpoint 无效、provider 已切换或 queued job 会恢复为 interrupted/retryable；该 key 被普通 prefs backup 排除，不继续旧 stream。
