...
| E01-C05-T15 | 实现 per-run runtime state 隔离。 | 第一片已完成：AI Chat inline panel、历史任务卡 snapshot、卡内送审和 scoped 恢复缓存按 `seminarSessionId` 隔离；剩余是独立详情页与 run-scoped composer 迁移到同一 store。 |
