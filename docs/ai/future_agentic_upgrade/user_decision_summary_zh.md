...
- 把当前带 snapshot、白板正文首片、首片异常送审按钮、异常送审内容计数、completed 卡内低负担保存动作、completed 卡内读者参与 composer、分歧快捷继续讨论入口、snapshot 子视图首片和 AI Chat scoped runtime 的历史任务卡继续升级为完整 AI Chat message part / run 卡片，补齐完整异常送审详情表、完整 contradiction 详情 tab、message part schema migration 和独立详情页 scoped store。
