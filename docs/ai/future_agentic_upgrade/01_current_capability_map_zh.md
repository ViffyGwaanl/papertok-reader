...
| Knowledge asset sync boundary | `KnowledgeSyncEnvelope` 已区分 `ai-draft`、`knowledge-card`、`derived-index`；`KnowledgeSyncPolicy` 默认排除 draft、derived cache 和含 secret-like payload 的 envelope；`KnowledgeCardStore` 中 draft/pending 不按用户资产同步，applied 且有 evidence 的卡片才进入 knowledge-card envelope；Knowledge sync/export 已能写安全 sync bundle、读取远端 bundle preview、把远端冲突送入 Review、把安全远端 incoming KnowledgeCard 降级为 pending Review，并在远端无 incoming/conflict 时受保护上传 sync bundle。 | 真正的同步管线还需要远端冲突 staging/import、双向合并、rollback 和同步状态 UI。 | E08 |
