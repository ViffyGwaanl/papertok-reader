  ...
  group('MarkdownMemoryStore', () {
    ...
    test('append + read + replace work for daily and long-term', () async {
      ...
      try {
        ...
        final older = DateTime(2026, 2, 24);
        await store.replace(longTerm: false, date: older, text: 'Older');
        ...
        // listDailyFileNames includes daily files.
        final dailyNames = await store.listDailyFileNames(limit: 10);
        expect(dailyNames, contains('2026-02-25.md'));
        expect(dailyNames, contains('2026-02-24.md'));

        // Search finds hits in both long-term and daily.
        final hits = await store.search('Hello', limit: 20);
        expect(hits.where((h) => (h['file'] as String) == 'MEMORY.md').length,
            greaterThanOrEqualTo(1));

        final dailyHits = await store.search('Older', limit: 20);
        expect(dailyHits.any((h) => (h['file'] as String) == '2026-02-24.md'),
            isTrue);
      } finally {
        try {
