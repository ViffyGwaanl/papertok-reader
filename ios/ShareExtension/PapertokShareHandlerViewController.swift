...
/// A vendored Share Sheet controller.
///
/// Why vendored?
/// - Upstream `ShareHandlerIosViewController` writes into App Group *root*.
/// - We want a namespaced inbox for safe cleanup + TTL.
///
/// Compatibility:
/// - Still writes the same `SharedMedia` JSON payload to App Group UserDefaults
///   key `ShareKey` so the existing `share_handler` drain logic keeps working.
@available(iOS 14.0, *)
...
class PapertokShareHandlerViewController: UIViewController {
  ...
  private func loadIds() {
    ...
    if let lastDot = shareExtensionBundleIdentifier.lastIndex(of: ".") {
      PapertokShareHandlerViewController.hostAppBundleIdentifier = String(shareExtensionBundleIdentifier[..<lastDot])
    } else {
      PapertokShareHandlerViewController.hostAppBundleIdentifier = shareExtensionBundleIdentifier
    }

    // AppGroupId from Info.plist build setting or fallback.
    let configured = (Bundle.main.object(forInfoDictionaryKey: "AppGroupId") as? String) ?? ""
    let trimmed = configured.trimmingCharacters(in: .whitespacesAndNewlines)
    if !trimmed.isEmpty {
      PapertokShareHandlerViewController.appGroupId = trimmed
    } else {
      PapertokShareHandlerViewController.appGroupId = "group.\(PapertokShareHandlerViewController.hostAppBundleIdentifier)"
    }
  }

  private func inboxEventDir() throws -> URL {
    let container = FileManager.default.containerURL(forSecurityApplicationGroupIdentifier: PapertokShareHandlerViewController.appGroupId)!
    let dir = container
  ...
  private func redirectToHostApp() {
    ...
    let url = URL(string: "ShareMedia-\(PapertokShareHandlerViewController.hostAppBundleIdentifier)://\(PapertokShareHandlerViewController.hostAppBundleIdentifier)?key=\(sharedKey)")
